 ...
 *  David Blythe, 1997
 *
 *  An example of creating points of light using several methods
 *  including normal points, points attenuated by the distance from
 *  the eye, and texture mapped points of light.
 *
 */
...
init(char *filename) {
	...
	if (components != 1 && components != 2) {
	    printf("warning: texture should be an l or la image\n");
	}
	if (components == 1) {
	    /* hack for RE */
	    int i;
...
/* v0 = v1*mat */
void
xform(float *v0, float *mat, float *v1) {
...
/* m0 = m1*m2 */
void
xformm(float *m0, float *m1, float *m2) {
    int i, j;
    for(i = 0; i < 4; i++) {
	for(j = 0; j < 4; j++) {
