 ...
 *  David Blythe, 1997
 *
 *  An example of how to draw a vapor trail (such as that left by an
 *  airplane).
 *
 */
...
draw_vapor(void) {
    int i;
    float lastx, size = 0.f;
    float intensity = .9f;
    glDepthMask(0);
    glEnable(GL_TEXTURE_2D);

    for(i = 0; i < vapors; i++) {
	struct vapor *v = vapor+i;
	float t = _time - v->time;

	if (i == 0) lastx = v->x;

	glPushMatrix();
	glTranslatef(lastx = v->x, v->y, v->z);
	glScalef(size = (.5*(t+40)/MAX_TIME), .5*(t+40)/MAX_TIME, 1.);
