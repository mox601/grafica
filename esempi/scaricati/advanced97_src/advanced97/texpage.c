...
loadtiles(void) {
    ...
    static ox = TILES*TILESIZE/2, oy = TILES*TILESIZE/2;  /* image origin */
    static ot = 0, os = 0;
...
printf("load %d %d  %d %d\n", nx+s/TILESIZE, ny, (s+os) % TSIZE, ot);
    ...
    glLoadIdentity();
    tr_x = (float)((x-TILES*TILESIZE/2) % TSIZE)/TSIZE;
    tr_y = (float)((y-TILES*TILESIZE/2) % TSIZE)/TSIZE;
    glTranslatef(tr_x, tr_y, 0.f);
