...
void whichObjectFunc(int data)
{
	...
	case 4:
	    curXform = &globalXform;
	    printf("Editing global transformation\n", data);
	    break;
    }
}
...
int main(int argc, char **argv)
{
    ...
    glutAddMenuEntry("Click the left mouse button and drag to manipulate", 0);
    glutAddMenuEntry("Push 'r' to rotate like a trackball", 0);
    glutAddMenuEntry("Push 't' to translate in the X-Y plane", 0);
    glutAddMenuEntry("Push 'T' to translate in the Z axis", 0);
    glutAddMenuEntry("Push 'X' to scale in X", 0);
    glutAddMenuEntry("Push 'X' to scale in Y", 0);
    glutAddMenuEntry("Push 'Z' to scale in Z", 0);
